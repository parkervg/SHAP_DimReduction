...
def assign_ranks(item_dict):
    ...
    sorted_list = [
        (key, val)
        for (key, val) in sorted(item_dict.items(), key=itemgetter(1), reverse=True)
    ]
    for i, (key, val) in enumerate(sorted_list):
        ...
        if len(same_val_indices) == 1:
            ...
            ranked_dict[key] = 1.0 * sum(same_val_indices) / len(same_val_indices)
    return ranked_dict


def correlation(dict1, dict2):
    avg1 = 1.0 * sum([val for key, val in dict1.iteritems()]) / len(dict1)
    avg2 = 1.0 * sum([val for key, val in dict2.iteritems()]) / len(dict2)
    numr, den1, den2 = (0.0, 0.0, 0.0)
...
def spearmans_rho(ranked_dict1, ranked_dict2):
    ...
    if len(ranked_dict1) == 0 or len(ranked_dict2) == 0:
        return 0.0
    x_avg = 1.0 * sum([val for val in ranked_dict1.values()]) / len(ranked_dict1)
    y_avg = 1.0 * sum([val for val in ranked_dict2.values()]) / len(ranked_dict2)
    num, d_x, d_y = (0.0, 0.0, 0.0)
